...
import { CreateOrEditCustomerDto, CustomerServiceProxy, CustomerInfoDto, SpouseDto, DependentDto, CustomerTypeDto, CustomerTypesServiceProxy, SourceReferralType, SourceReferralTypeDto, SourceReferralTypeServiceProxy, DLStatesServiceProxy, DLState, Dependent } from '@shared/service-proxies/service-proxies';
...
export class CustomerInfoComponent extends AppComponentBase implements OnInit {
  ...
  public ngOnInit() {
    ...
    this._activatedRoute.params.subscribe(parms => {
      if (parms.id || this.customer.id && this.customer.id > 0) {
