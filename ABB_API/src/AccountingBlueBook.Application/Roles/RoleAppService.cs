...
namespace AccountingBlueBook.Roles
{
    ...
    public class RoleAppService : AsyncCrudAppService<Role, RoleDto, int, PagedRoleResultRequestDto, CreateRoleDto, RoleDto>, IRoleAppService
    {
        ...
        public async Task CreateNewAsync(RoleDto input)
        {
           // var IpAddress = input.IpAddress.Count() > 0 ? string.Join(",", input.IpAddress) : null;
            var role = ObjectMapper.Map<Role>(input);
            role.SetNormalizedName();
        ...
        public async Task UpdateNewAsync(RoleDto input)
        {
            var role = await GetRole(input.Id);
        ...
        {
            var role = await _roleRepository.GetAll().FirstOrDefaultAsync(a => a.Id == Id);
            if (role == null)
        ...
        public async Task<GetRoleForEditOutput> GetRoleForEdit(EntityDto input)
        {
            var permissions = PermissionManager.GetAllPermissions();
            var role = await _roleManager.GetRoleByIdAsync(input.Id);
          //  List<string> resultList = role.IpAddress.Split(",").ToList();
            var grantedPermissions = (await _roleManager.GetGrantedPermissionsAsync(role)).ToArray();
            var roleEditDto = ObjectMapper.Map<RoleDto>(role);
        ...
        public ListResultDto<FlatPermissionWithLevelDto> GetAllPermissionsWithLevel()
        {
            var permissions = PermissionManager.GetAllPermissions();
        ...
        private void AddPermission(Permission permission, IReadOnlyList<Permission> allPermissions, List<FlatPermissionWithLevelDto> result, int level)
        {
            ...
            {
                // Recursively call the method to process child permissions
